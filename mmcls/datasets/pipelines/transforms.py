...
from configs._base_.datasets.voc_bs16 import img_norm_cfg
...
class Eqresize(object):
    ...
    def _resize_img(self, results):
        for key in results.get('img_fields', ['img']):
            ...
            height, width = self.size
            scale = min(height*1.0/org_h, width*1.0/org_w)
            if scale !=1 :
                img = mmcv.imresize(
                    img,
                    size=(int(org_w*scale + 0.5), int(org_h*scale + 0.5)),
                    interpolation=self.interpolation,
                    return_scale=False,
                    backend=self.backend)
            h, w = img.shape[:2]
